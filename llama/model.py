...
class Transformer(nn.Module):
    def __init__(self, params: ModelArgs, args):
        ...
        self.freqs_cis = precompute_freqs_cis(self.params.dim // self.params.n_heads, args.max_seq_len * 2)
        self.freqs_cis2 = precompute_freqs_cis(self.params.dim // self.params.n_heads, args.max_cap_len * 2)
    ...
    def forward(self, data, inference=False):
        ...
        cap_id = data['text_id']['cap'].cuda()
        cap_label = data['label']['cap'].cuda()
        cap_video_start = data['video_start']['cap'][0]

        bsz, n_options, seqlen = vqa_id.shape
        _, _, caplen = cap_id.shape

        vqa_id = vqa_id.reshape(-1, seqlen)
        vqa_label = vqa_label.reshape(-1, seqlen)
        vqa_label = vqa_label[:, 1:].flatten()

        cap_id = cap_id.reshape(-1, caplen)
        cap_label = cap_label.reshape(-1, caplen)
        cap_label = cap_label[:, 1:].flatten()

        with torch.no_grad():
            vqa_h = self.tok_embeddings(vqa_id)
            if not inference:
                cap_h = self.tok_embeddings(cap_id)

        freqs_cis = self.freqs_cis.to(vqa_h.device)
        freqs_cis = freqs_cis[:seqlen]
        if not inference:
            freqs_cis2 = self.freqs_cis2.to(cap_h.device)
            freqs_cis2 = freqs_cis2[:caplen]
        ...
        if not inference:
            mask2 = None
            mask2 = torch.full((1, 1, caplen, caplen), float("-inf"), device=cap_h.device)
            mask2 = torch.triu(mask2, diagonal=0 + 1).type_as(cap_h)
        ...
        if not inference:
            cap_h = cap_h.clone()
            cap_h[:, cap_video_start:cap_video_start + self.max_feats] = video_feature
        ...
        for i, layer in enumerate(self.layers[-1 * self.adapter_layer:]):
            ...
            if not inference:
                cap_h = layer(cap_h, start_pos, freqs_cis2, mask2, adapter[i].half(), cap_video_start)

        vqa_h = self.norm(vqa_h)
        vqa_output = self.output(vqa_h)
        vqa_output = vqa_output[:, :-1, :].reshape(-1, self.vocab_size)
        vqa_loss = self.train_criterion(vqa_output, vqa_label)

        if not inference:
            cap_h = self.norm(cap_h)
            cap_output = self.output(cap_h)
            cap_output = cap_output[:, :-1, :].reshape(-1, self.vocab_size)
            cap_loss = self.train_criterion(cap_output, cap_label) * self.args.cap
